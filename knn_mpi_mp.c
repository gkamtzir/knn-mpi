  ...
  t = atoi(argv[5]);
	...
	while(1)
	{
		...
		if (rank % 2 == 0)
		{
			...
			MPI_Send(&(target[0][0]), data_per_process * d, MPI_DOUBLE, next, tag, MPI_COMM_WORLD);
			MPI_Send(target_id, data_per_process, MPI_INT, next, tag, MPI_COMM_WORLD);

			MPI_Recv(&(temp[0][0]), data_per_process * d, MPI_DOUBLE, previous, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			MPI_Recv(temp_id, data_per_process, MPI_INT, previous, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
